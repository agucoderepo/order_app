...
function lineTotal(unitPrice: number, quantity: number, discountPct: number): number {
...
export default function OrderModal({ order, onClose, onSaved, toast }: Props) {
  ...
  function addItem() {
    setItems((prev) => [...prev, { product_id: '', quantity: '', discount: '0' }]);
  }
  ...
  function validate(): string | null {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (isNaN(disc) || disc < 0 || disc > 100) return `Row ${i + 1}: discount must be between 0 and 100`;
    }
    const productIds = items.map((r) => r.product_id);
    if (new Set(productIds).size !== productIds.length) return 'Duplicate products in order';
    return null;
  }
  ...
  // For editing, use the snapshotted unit_price; for creating, use current product price
  ...
  function getUnitPrice(productId: string): number {
    if (editing && itemUnitPriceMap.has(productId)) return itemUnitPriceMap.get(productId)!;
    const p = productMap.get(productId);
    return p ? Number(p.price) : 0;
  }

  const grandTotal = items.reduce((sum, r) => {
    const unitPrice = getUnitPrice(r.product_id);
    const qty = parseFloat(r.quantity);
    const disc = parseFloat(r.discount) || 0;
    if (!r.product_id || isNaN(qty)) return sum;
    return sum + lineTotal(unitPrice, qty, disc);
  }, 0);

  return (
    <Modal
      title={editing ? 'Edit order' : 'New order'}
      subtitle={editing ? `#${order!.id.slice(0, 8)} · ${order!.client.name}` : 'Create a manual order'}
      onClose={onClose}
      width={680}
    >
      <form onSubmit={submit}>
        {loadingData ? (
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: editing ? '1fr 1fr' : '1fr 1fr', gap: 12 }}>
              {!editing && (
                <div className="field" style={{ gridColumn: '1 / -1' }}>
                  <label>Client</label>
                  <select value={clientId} onChange={(e) => setClientId(e.target.value)} required style={selectStyle(!!clientId)}>
              ...
              {editing && (
                <div className="field">
                  <label>Status</label>
                  <select value={status} onChange={(e) => setStatus(e.target.value as OrderStatus)} style={selectStyle(true)}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              {/* Column headers */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 80px 28px', gap: 6, marginBottom: 4, paddingRight: 2 }}>
                {['Product', 'Qty', 'Disc. %', 'Subtotal', ''].map((h) => (
                  <div key={h} style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '.06em' }}>
                    {h}
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {items.map((row, idx) => {
                  ...
                  const unitPrice = getUnitPrice(row.product_id);
                  const qty = parseFloat(row.quantity);
                  const disc = parseFloat(row.discount) || 0;
                  const total = row.product_id && !isNaN(qty) ? lineTotal(unitPrice, qty, disc) : null;
                  ...
                  return (
                    <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 80px 28px', gap: 6, alignItems: 'center' }}>
                      <select
                        ...
                        style={selectStyle(!!row.product_id)}
                      >
                        <option value="">Select product...</option>
                        {products.map((p) => (
                          <option key={p.id} value={p.id}>
                            {p.name} ({p.unit}) — ${Number(p.price).toFixed(2)}
                          </option>
                        ))}
                      </select>
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        />
                        <span style={{ position: 'absolute', right: 7, top: '50%', transform: 'translateY(-50%)', fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', pointerEvents: 'none' }}>
                          %
                        </span>
                      </div>
